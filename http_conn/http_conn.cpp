...
const char* error_404_form = "The requested URL was not found on the server.\n";
const char* error_500_title = "Internal Server Error";
const char* error_500_form = "The server encountered an internal error and was unable to complete your request.\n";

locker m_lock;
map<string,string> users;

// 1
void http_conn::initmysql_ret(connection_pool *connpool)
{
    MYSQL* mysql = NULL;
...
void modfd(int epollfd,int fd,int ev,int TGIGMode)
{
    epoll_event event;
    event.data.fd = fd;
...
bool http_conn::read_once()
{
    ...
    if(0 == m_TRIGMode)
    {
        bytes_read = recv(m_sockfd,m_read_buff + m_read_idx,READ_BUFF_SIZE - m_read_idx,0);
...
{
    return add_response("Connection:%s\r\n",(m_linger == false) ? "close" : "keep-alive");
}
